...
import { cacheResultsSearch } from "./utils/searchSlice";
...
const Header = () => {
  ...
  // show suggestions field bind with input
  const[showSuggestions,setShowSuggestions]=useState(false)

  // getting the cacheresult from redux store by subscribe to the store
  const searchCache = useSelector((store)=>store.cachesearch)
  ...
  useEffect(() => {
    ...
    // making the suggestion smart by caching
  ...
  const getSearchQuery = async () => {
    try {
      ...
      // update the cache result in redux
      
      dispatch(cacheResultsSearch(
        {
          [searchQuery]:json[1]
        }
      ))
...
// toggling the side menu while clicking the toggle function is coming from the redux store
 ...
 {/* show input suggestions only if showSuggestions is true otherwise hide it . */}
  {showSuggestions && (
